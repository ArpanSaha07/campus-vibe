...
- [ ] **P1** **Re-run the full tier and get it green past the Trivy gate.** BUG-016 is fixed and the frontend image is verified locally to scan clean of fixable HIGH *and* CRITICAL, but that has not run on GitHub, and the **backend** image has not been rescanned locally at all. Still **expected to fail on [BUG-001](../bugs/bugs.md#bug-001)** once it gets past the gate; that is the correct signal, not a pipeline defect.
...
- [ ] **P0** **BLOCKER, NOW LIVE — `main` cannot be merged into.** Branch protection went on 2026-08-07, so this stopped being a prediction. Measured 2026-08-05 with Docker running: `./mvnw verify` was **40 tests, 39 pass** (should now be 42/41 with the actuator cases; not re-measured), and the single failure is `SearchIT.semanticSearchMatchesMeaningWithoutSharedKeywords:163`. Nothing else in the full tier is red. Every PR runs the full tier and `CI` is required, so **no PR can merge** — including the nine Dependabot ones — until you either fix [BUG-001](../bugs/bugs.md#bug-001) or quarantine **just that method** with `@Disabled("BUG-001: …")`. Quarantining the whole class would also lose the 6 passing search tests.
...
- [ ] **P2** **Tighten the Trivy gate** in `_docker.yml` once *both* base-image baselines are known. It reports fixable HIGH+CRITICAL and fails only on fixable CRITICAL. Since [BUG-016](../bugs/fixed_bugs.md#bug-016) the **frontend** image measures zero fixable HIGHs locally, so half the noise floor is now a measurement rather than a guess; the backend image has not been measured. Move the gate to HIGH after one clean full-tier run covers both.
