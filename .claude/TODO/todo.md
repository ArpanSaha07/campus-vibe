...
- [ ] **Semantic search returns nothing for meaning-only matches.** Pre-existing; 1 of 40 tests failing. Embedding *writes* are proven fine, so the fault is in `SearchRepository.hybridSearchEventIds`. **Passed in two full local runs on 2026-09-12**, unexplained, so still open until a green GitHub run. ([BUG-001](../bugs/bugs.md#bug-001))
