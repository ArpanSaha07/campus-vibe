...
- [ ] **P1** **Re-run the full tier and get it green past the health probe.** BUG-015 is fixed and both jobs are verified locally, but nothing after the health check has ever executed on a runner — the compose secret fail-fast assertion, the production frontend image build and the Trivy gate are all still unproven on GitHub. Still **expected to fail on [BUG-001](../bugs/bugs.md#bug-001)** once it gets that far; that is the correct signal, not a pipeline defect.
...
**2026-08-07 — The full CI tier ran for the first time and found a real defect ([BUG-015](../bugs/fixed_bugs.md#bug-015)).**
