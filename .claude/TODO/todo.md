...
Last updated: **2026-08-19** · Branch: `feature/club-governance`
...
- [ ] **P1** **User profiles have no backend at all, and a whole editor now writes to a stub.** `/profile/edit` shipped 2026-08-19 as a complete UI over memory: `saveProfile` and `saveNotificationPreferences` in `frontend/app/lib/profile.ts` resolve without calling anything, so every section saves, confirms, and is gone on reload. This is the single largest piece of unbacked frontend in the project.

  **The work:** a migration for the profile columns (`bio`, `faculty`, `degree`), the two collections (`subjects`, `interests`) and the three social links, plus the two visibility flags; a `UserProfileDTO` **added to `contracts/api-dto-fields.json`**, since both contract tests assert against it; `GET`/`PATCH /api/v1/users/me/profile`; and a separate store for the five email preferences, which are not profile data and should not share a row with it.

  **Do not fold these onto `UserDTO`.** `api-contract.test.ts` keys on `Record<keyof User, true>`, so the frontend's `User` cannot gain a field until the backend serialises it — that guard is what pushed the shape into `UserProfile` in the first place, and it should keep them apart. The frontend shape to mirror is `UserProfile` / `NotificationPreferences` in `frontend/app/types/index.ts`; when the endpoints exist, the two seam functions are the only frontend code that changes.
- [ ] **P2** **Nothing can change an account's email address.** `/profile/edit/account` renders the field and its Save commits locally. Needs a confirm-the-new-address round trip, not a straight update — `auth_tokens` (V11) already carries the machinery, and the address is the login identifier, so an unverified change locks the account out.
...
- [ ] **P2** **Profile photo upload.** `ProfileAvatar` draws the first initial and `/profile/edit` says in words that uploads are not available, rather than showing a disabled button. There is no avatar field on `User` to fall back from. S3 is already wired for club logos and event banners, so this is the same path with a different owner.
- [ ] **P2** **The public profile view does not exist.** `showInterests` and `showSocialLinks` are editable and govern how a profile looks *to other people*; `/profile` only ever shows you your own, and you always see all of your own. Until someone else can open a profile, both switches control nothing. Decide the route (`/users/[id]`?) and what a visitor may see.
- [ ] **P3** **Interests are collected and never used.** The catalogue exists and the picker works, but nothing reads the chosen values — no filtering, no ranking, no suggestion. The copy on that screen promises we will use them to suggest clubs and events, which is currently untrue. Natural pair with the search work, since `search` already has the embedding machinery.
...
- [ ] **P2** **Write the user-profile implementation doc** — `docs/architecture/user-profiles.md`, and add it to `docs/README.md` and `scripts/docs-map.json`. Two commits on 2026-08-19 added roughly 1,900 lines across `/profile`, `/profile/edit` and their types with no doc behind them. The reasoning worth keeping is mostly about *constraints*: why the writable half of a profile is a separate interface from `User` (the contract test is exhaustive over `keyof User`), why `normaliseProfileLink` checks the scheme before assuming https, why interests are a closed list and subjects are not, and why nothing persists yet.
- [ ] **P3** **Check the McGill faculty list and the interest catalogue** in `frontend/app/lib/profile-options.ts`. Twelve faculties and schools, transcribed from the university's own listing rather than fetched — and faculties do get renamed, Dentistry having become *Dental Medicine and Oral Health Sciences* recently. The 12-category, ~80-entry interest catalogue underneath it was invented for a campus audience and has had no product review at all.
- [ ] **P3** **Look at `/profile` and `/profile/edit` in a browser, at both breakpoints.** Neither has ever been rendered — both commits were shipped on a green `verify` alone, which proves they compile and not that they look right. The two-column split at `lg`, the sticky identity card, the mobile rail that scrolls sideways, and the interests grid at ~80 pills are the parts most likely to be wrong.
