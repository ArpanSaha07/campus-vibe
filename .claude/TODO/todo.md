...
- [ ] **P1** *(Step 6)* Set a hard monthly budget cap on the OpenAI project — the only control that bounds the loss from a leaked key. **More pressing since 2026-09-16:** the planner makes a generative call per message, bounded per user by its daily quota but not in total. Use **separate OpenAI projects per environment**.
...
- [ ] **P2** **Look at the planner in a browser at 390px, and at the states only tests have seen** — the 15-of-15 eviction notice, a 429, a provider failure, the homepage card hand-off. Seen on 2026-09-16 against the real backend and key, at desktop width: an events answer, a clubs answer, both follow-up chip kinds, a reload and a declined prompt injection. Natural moment: the first run on the deployed site.
- [ ] **P2** **Watch the planner's stream behind the production load balancer and nginx.** The backend sends `X-Accel-Buffering: no` and a keep-alive comment every 15 seconds for exactly that path, and neither has been seen behind a real proxy: locally there is none. After deploying, send one message and confirm the intro arrives in pieces rather than all at once, and read the `planner.answer` log line for dropped picks ([`ai-planner.md`](../docs/architecture/ai-planner.md)).
- [ ] **P3** **An evaluation set for the planner's retrieval and prompt.** `PlannerRanker`'s weights are hand-set and the first live answer's intro promised two workshops over one card. A dozen prompts with the events a person would expect, run against a seeded database, before tuning weights or rewording `planner-system.txt`.
