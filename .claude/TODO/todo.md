...
- [ ] **P2** **Set `AUTH_RATE_LIMIT_TRUST_XFF=true` in the EB environment**, and only there. Behind Cloudflare every request arrives from a proxy address, so the limiter would otherwise treat the whole internet as one client. It must stay `false` anywhere the app is directly reachable, where it would let a caller forge an IP per request.
- [ ] **P3** **PostgreSQL 15 reaches end of life in November 2027.** Production will be pinned to 15.x to match `pgvector/pgvector:pg15` in Compose, both Testcontainers suites and `_database.yml`. Moving majors means all four pins plus an RDS upgrade plus a local volume rebuild — schedule it, do not discover it.
