...
- [ ] **P1** **Rate-limit the search endpoints too** ([BUG-005](../bugs/bugs.md#bug-005)). The auth limiter above is deliberately scoped to `/api/v1/auth/**`; search is public for different reasons (OpenAI spend, not credential guessing) and wants its own budget. `AuthRateLimiter` is a working template — but note it keys on IP only, and the search case may want a global spend ceiling as well.
...
- [ ] **P2** **Choose and test a production mail provider.** The logging sender is right for local work and CI, but `SmtpMailSender` has never sent a message through a real server, so nothing about production delivery is proven. Needs a provider decision (SES matches the planned EB deploy), a verified sender domain, SPF/DKIM, and one real send. Trigger: before any real-user launch — until then the reset flow works only for people who can read the server log.
- [ ] **P3** **Decide whether to turn on `AUTH_REQUIRE_VERIFIED_EMAIL`.** Blocked on the item above: switching it on before mail actually sends would lock every new user out of their own account.
...
- [ ] **P3** Pin the bcrypt cost factor explicitly (`SecurityConfig.java:18` takes the library default — fine today, but it is a security parameter left implicit).
- [ ] **P3** Add a Content-Security-Policy to the frontend. Reduces the blast radius of the `localStorage` token while it is still there.
