...
| [`ai-planner.md`](architecture/ai-planner.md) | The AI planner end to end: the `/planner` chat page and its layout-level provider; the backend that keeps 15 chats and 15 messages a day, retrieves only events that have not ended, streams the intro out of one strict-JSON completion and keeps only picks it offered; what is stored, counted or refunded for each outcome | ✅ Written with the code 2026-09-16 · backend uncommitted, seen working on the local stack with the real key; not deployed |
...
| [`connecting-s3.md`](architecture/connecting-s3.md) | Runbook 3 of 4 — the bucket property and CORS removal (both done 2026-09-12), why the role holds `ListBucket`, the nginx body limit, the duplicate policy, verifying the first upload | 📋 Checklist · account changed 2026-09-12 |
...
| [`ADR-014`](decisions/ADR-014-one-embedding-call-per-search-via-the-cache.md) | One embedding call per search, by joining an in-flight query in `QueryEmbeddingCache` rather than adding a combined search endpoint | 📝 Proposed 2026-09-14 — awaiting Arpan | Dropdown users hit the search rate limit · a third search target joins the dropdown · the backend runs more than one instance. No rule yet: triggers are mirrored into `rules/` on acceptance. |
| [`ADR-015`](decisions/ADR-015-event-banner-is-the-first-photo.md) | An event's banner is its first photo, chosen by the club and live on the event page · ten photos per event · order stored in `sort_order` with a deferred unique key · the platform-approval flow narrowed to the homepage carousel | 📝 Proposed 2026-09-15 — awaiting Arpan | The homepage carousel request is built — it needs its own pointer, not position 0 · a gallery order separate from the banner. `rules/backend-clubs.md`, `rules/db-migrations.md`. |
| [`ADR-016`](decisions/ADR-016-media-urls-versioned-by-key-hash.md) | Media URLs carry a hash of their stored key as the last path segment, so a changed image gets a new URL · why not `?v=`, uncached responses, or keys in URLs | 📝 Proposed 2026-09-15 — awaiting Arpan | Media moves behind a CDN or presigned URLs · Next changes its rule on query strings for local images. `rules/frontend.md`. |
| [`ADR-017`](decisions/ADR-017-api-https-through-cloudfront-not-a-load-balancer.md) | Retiring the load balancer: API HTTPS through CloudFront in front of a single-instance environment, origin locked to the CloudFront prefix list · why Let's Encrypt on the instance and Cloudflare were rejected · the eight-step order, with `ClientIpResolver` fixed first, and the rollback | 📝 Proposed 2026-09-15 — awaiting Arpan | Nothing yet — awaiting a decision. Once accepted: a second instance is needed · deploy downtime becomes user-visible · traffic nears CloudFront's free tier · a review requires an encrypted origin leg. |
| [`ADR-018`](decisions/ADR-018-google-renders-and-styles-its-own-button.md) | The Continue with Google control is Google’s own button, rendered and styled by GIS · why the hidden-button click proxy had to go, and why an invisible overlay and the OAuth code flow were rejected | ✅ Accepted 2026-09-15 | Google changes the button API or GIS is deprecated · the design rejects the mismatch in the auth modal (the overlay is the recorded fallback) · a surface needs a control the button API cannot express. `rules/frontend.md`. |
| [`ADR-019`](decisions/ADR-019-planner-answer-is-intro-plus-typed-picks.md) | A planner answer is an intro plus typed picks of one kind, not prose with inline markers · why the layout is rendered from data, and what streams | ✅ Accepted 2026-09-16 | An answer needs two rows · the provider cannot return structured picks after a streamed intro. `docs/architecture/ai-planner.md`. |
| [`ADR-020`](decisions/ADR-020-event-attendable-until-its-end-time.md) | An event is still attendable until its required end time, at most 14 days after its start, one definition everywhere · why not a default duration or an optional end | ✅ Accepted 2026-09-16 | A club needs an event longer than 14 days · events outside Montreal's zone · event lifecycle status lands. `rules/backend-clubs.md`, `rules/frontend.md`. |
| [`ADR-021`](decisions/ADR-021-planner-streams-its-intro-out-of-strict-json.md) | The planner gets intro and picks from one streamed completion with a strict JSON schema, decoding the intro out of the partial JSON · why two calls and a delimited text-then-JSON reply were rejected | 📝 Proposed 2026-09-16 — awaiting Arpan | A second provider cannot stream a strict schema in property order · streaming each reason becomes a request · the answer log shows answers failing to parse or dropping picks. `docs/architecture/ai-planner.md`, `rules/backend-java.md`. |
