...
| [`ai-planner.md`](architecture/ai-planner.md) | The `/planner` chat page: why its state lives in a layout-level provider, the streamed `delta` / `done` reply read through `apiFetchResponse`, the answer rendered from typed picks, the 15-chat and 15-message limits as the page shows them · **the backend it calls is unbuilt** | ⚠ **Frontend only** · written with the code 2026-09-16; signed-in users see the unavailable state until the planner endpoints ship |
...
| [`ADR-019`](decisions/ADR-019-planner-answer-is-intro-plus-typed-picks.md) | A planner answer is an intro plus typed picks of one kind, not prose with inline markers · why the layout is rendered from data, and what streams | 📝 Proposed 2026-09-16 — awaiting Arpan | An answer needs two rows · the provider cannot return structured picks after a streamed intro. `docs/architecture/ai-planner.md`. |
| [`ADR-020`](decisions/ADR-020-event-attendable-until-its-end-time.md) | An event is still attendable until its required end time, at most 14 days after its start, one definition everywhere · why not a default duration or an optional end | 📝 Proposed 2026-09-16 — awaiting Arpan | A club needs an event longer than 14 days · events outside Montreal's zone · event lifecycle status lands. `rules/backend-clubs.md`, `rules/frontend.md`. |
