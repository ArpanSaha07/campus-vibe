...
| [ADR-015](ADR-015-event-banner-is-the-first-photo.md) | 2026-09-15 | ✅ Accepted 2026-09-15 | An event holds up to ten photos ordered by `sort_order` (V34), and position 0 is its banner, chosen by the club and live on the event page · rejected a `banner_image_key` column, an unordered table, and building the platform-approval flow first — which stays, for the homepage carousel | `EventService.makeBanner`, `V34__add_event_images_sort_order.sql` — 2026-09-15 |
| [ADR-016](ADR-016-media-urls-versioned-by-key-hash.md) | 2026-09-15 | 📝 Proposed — awaiting Arpan | Media URLs end in a hash of the stored key as a path segment, so a URL changes exactly when its image does · rejected `?v=` (next/image throws on it), uncached image responses, and addressing by key | `adapters.ts` `mediaVersion`, `next.config.ts` rewrites — 2026-09-15 ([BUG-056](../../bugs/fixed_bugs.md#bug-056)) |
| [ADR-017](ADR-017-api-https-through-cloudfront-not-a-load-balancer.md) | 2026-09-15 | ✅ Accepted 2026-09-22 | When the load balancer is retired, API HTTPS terminates at CloudFront (ACM cert in `us-east-1`, caching off, HTTP→HTTPS at the edge) in front of a single-instance environment whose port 80 admits only the CloudFront origin-facing prefix list · `ClientIpResolver` fixed first · rejected keeping the ALB (~$25–30/month balancing one instance), Let's Encrypt on the instance, and Cloudflare (moves DNS off Namecheap) | — in progress 2026-09-22, [spec](../../specs/2026-09-22-cloudfront-single-instance.md) |
| [ADR-018](ADR-018-google-renders-and-styles-its-own-button.md) | 2026-09-15 | ✅ Accepted 2026-09-15 | The Continue with Google control **is** Google’s own button, rendered and styled by GIS through `theme` / `size` / `shape` / `text` / `width` alone · the hidden-button click proxy is deleted, because GIS began serving a cross-origin iframe and nothing in a page can click into one · rejected an invisible overlay of Google’s button over ours (the same fragile coupling, silent when the hit area drifts) and the OAuth code flow (a new endpoint, and a client secret where there is only a public client id) | `GoogleAuthButton.tsx` — 2026-09-15 ([BUG-057](../../bugs/fixed_bugs.md#bug-057)) |
| [ADR-019](ADR-019-planner-answer-is-intro-plus-typed-picks.md) | 2026-09-16 | ✅ Accepted 2026-09-16 | A planner answer is intro text plus typed picks `{kind, id, reason}` of one kind, the intro streamed and the picks arriving in the closing `done` frame · rejected prose with inline `[[event:id]]` markers (layout left to the model) and a fully structured, unstreamed reply | [`ai-planner.md`](../architecture/ai-planner.md) — frontend 2026-09-16 (`2b8cfe4`); backend 2026-09-16, uncommitted |
| [ADR-020](ADR-020-event-attendable-until-its-end-time.md) | 2026-09-16 | ✅ Accepted 2026-09-16 | Every event has a required end, after its start and at most 14 days later, and is still attendable while `end_time > now()` — one comparison for search, `?upcoming=true`, the dashboards, My events and the Happening now badge · rejected a default duration, start-only filtering and an optional end · event times shown and entered in America/Toronto | V35, `EventService`, `SearchRepository`, `lib/event-time.ts`, `lib/event-zone.ts` — 2026-09-16 (`95418a1`) |
| [ADR-021](ADR-021-planner-streams-its-intro-out-of-strict-json.md) | 2026-09-16 | 📝 Proposed — awaiting Arpan | The planner makes one streamed completion per message with a strict JSON schema, `intro` first, and decodes the intro out of the partial JSON for `delta` frames; the stored answer comes from a full parse · rejected two calls (double cost, an intro that can disagree with its picks) and plain text then a delimited JSON block (no enforcement) | `OpenAiLlmClient`, `IntroStreamReader`, `PlannerAnswers` — 2026-09-16, uncommitted |
