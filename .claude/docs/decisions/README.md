...
| [ADR-017](ADR-017-api-https-through-cloudfront-not-a-load-balancer.md) | 2026-09-15 | 📝 Proposed — awaiting Arpan | When the load balancer is retired, API HTTPS terminates at CloudFront (ACM cert in `us-east-1`, caching off, HTTP→HTTPS at the edge) in front of a single-instance environment whose port 80 admits only the CloudFront origin-facing prefix list · `ClientIpResolver` fixed first · rejected keeping the ALB (~$25–30/month balancing one instance), Let's Encrypt on the instance, and Cloudflare (moves DNS off Namecheap) | — not yet built; the ALB stays for the demo |
