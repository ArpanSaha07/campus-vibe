...
| [ADR-019](ADR-019-planner-answer-is-intro-plus-typed-picks.md) | 2026-09-16 | 📝 Proposed — awaiting Arpan | A planner answer is intro text plus typed picks `{kind, id, reason}` of one kind, the intro streamed and the picks arriving in the closing `done` frame · rejected prose with inline `[[event:id]]` markers (layout left to the model) and a fully structured, unstreamed reply | [`ai-planner.md`](../architecture/ai-planner.md) — frontend half, 2026-09-16; the backend is unbuilt |
| [ADR-020](ADR-020-event-attendable-until-its-end-time.md) | 2026-09-16 | 📝 Proposed — awaiting Arpan | Every event has a required end, after its start and at most 14 days later, and is still attendable while `end_time > now()` — one comparison for search, `?upcoming=true`, the dashboards, My events and the Happening now badge · rejected a default duration, start-only filtering and an optional end · event times shown and entered in America/Toronto | V35, `EventService`, `SearchRepository`, `lib/event-time.ts`, `lib/event-zone.ts` — 2026-09-16, uncommitted |
