...
**Code as of:** `31c7abb` · 2026-09-10 · branch `feature/club-governance`. The club ownership spine, the create-club page rebuild, its review edits and its test coverage are all committed. Uncommitted: a club proposal now carries the four contact links, the link rule became shared and is enforced on every write path, and Instagram is collected as a handle.
...
1. **Review and commit the contact-links unit** — verified, uncommitted, on top of `31c7abb`. A proposal carries the four links (`V33`), approval writes them onto the club, and the review queue shows them. The link rule moved to `common/WebLinks` and now runs on **both** club write paths, which closed [BUG-048](bugs/fixed_bugs.md#bug-048) — a defect that predated this work and was about to widen from admins to every signed-in user. [BUG-049](bugs/fixed_bugs.md#bug-049) fixed on the way. Frontend suite 243, backend units 73, `ClubCreationFlowIT` 15. Spec: [`2026-09-10-proposal-social-links.md`](specs/2026-09-10-proposal-social-links.md).
2. **P1 — seed a club's `official_email` from the create form.** The next unit, decisions already taken (Arpan, 2026-09-10): the contact email **seeds** `official_email` at creation and on approval, the two stay independent afterwards, and **editing stays platform-admin-only** — no owner/club-admin edit UI, so §6's trust anchor survives. No migration. It crosses `ClubCreateRequest`'s recorded *no such field* rationale, so `/start` first; [`todo.md`](TODO/todo.md) under Club governance carries the shape.
