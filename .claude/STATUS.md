...
**Code as of:** `85b79b4` · 2026-09-10 · branch `feature/club-governance`. The club ownership spine, the create-club page rebuild, and the contact links a club proposal carries are all committed — the last as `85b79b4`. Uncommitted: a club's official email, now seeded at creation on both paths.
...
1. **Review and commit the official-email seeding** — verified, uncommitted, on top of `85b79b4`. `official_email` could not be set until the club already existed, so every club started with no recovery address; both creation paths now seed it from the contact email the form already collects. No migration. **Editing stays platform-admin-only**: Arpan asked for owner and club-admin editing, was shown that being un-editable by the club is what makes the address a trust anchor (§6), and chose to keep it closed — **no owner/admin edit UI exists and none should be built.** `/manage/[clubId]` needed no change. Frontend 249 in 22 suites, backend units 73, `ClubCreationFlowIT` 19. Spec: [`2026-09-10-official-email-seeded-at-creation.md`](specs/2026-09-10-official-email-seeded-at-creation.md).
